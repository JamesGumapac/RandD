 ...
 */
 define(['N/file', 'N/search', 'N/record', 'N/redirect', 'N/runtime', 'N/ui/serverWidget', 'N/url', '../lib/moment.min', './se_lib_generalWOSchedule'],

 (file, search, record, redirect, runtime, ui, url, moment, gwosLib) => {
     ...
     const firstPage = context => {
         ...
         try {
             ...
             var searchId = script.getParameter('custscript_gwos_search_v4');
             var editedFields = script.getParameter('custscript_gwos_editfields_v4');
...
/*                 if (par.custpage_operation) {
                 ...
                 */
                 ...
                 // Get result limit
                 var script = runtime.getCurrentScript();
                 var limit = par.custpage_page == 'csv' ? '' : parseInt(script.getParameter('custscript_gwos_linelimit_v4'));
...
*/
				if(par.custpage_operation)
					limit = '';
				var operationIds = gwosLib.getPendingOperations( ss.filters, limit );
                         ...
                         searchResults[0].columns.forEach((col, i) => {
							...
							var displayType = label.match(/display\[/gi)? label.match(/display\[.*?\]/gi)[0].replace(/display\[|\]/gi, ''): '';
                             if (label != woPriority1_2FormulaLabel) { // Dont add the WOPrio1 + WOPrio2 to the UI cols
                                 ...
                                 label = label.match(/id\[/gi) ? label.replace(/id\[.*?\]/gi, '').trim() : label;// Remove id code
								 label = label.match(/display\[/gi)? label.replace(/display\[.*?\]/gi, '').trim(): label;// Remove display type code
                                 var type = ui.FieldType.TEXT;
                                 if (id.match(/wopriority|weekendhrs|cnt_board_thickness|cntm_boardpitch/gi))
                                     type = ui.FieldType.FLOAT;
                                 else if (id.match(/wo_scheddue/gi))
...
log.debug('id', id);
             ...
             form.clientScriptModulePath = './se_cs_generalWOschedule_v3.1.0';
