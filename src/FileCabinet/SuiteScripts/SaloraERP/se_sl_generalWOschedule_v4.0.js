 ...
 (file, search, record, redirect, runtime, ui, url, moment) => {
     ...
     const firstPage = context => {
         ...
         try {
             ...
             var searchId = script.getParameter('custscript_gwos_search_param_v4');
             var editedFields = script.getParameter('custscript_gwos_editfields_param_v4');
...
//                         {name: 'internalid', summary: 'MIN'}
                 ...
                 */
                 ...
                 // Get result limit
                 var script = runtime.getCurrentScript();
                 var limit = par.custpage_page == 'csv' ? '' : parseInt(script.getParameter('custscript_gwos_linelimit_param_v4'));
                         ...
                         searchResults[0].columns.forEach((col, i) => {
                             ...
                             if (label != woPriority1_2FormulaLabel) { // Dont add the WOPrio1 + WOPrio2 to the UI cols
								...
								if(id == 'hiddenfield')
                                 ...
                                 // Added by jeromemorden | 01/20/22
                                 if (id.match(/cntm_priority|weekend_hours/gi))  //added priorty|
                                     sublistFields[id].updateDisplaySize({
                                         height: 60,
                                         width: 4
                                     });
                                 if (id.match(/cntm_wo_sched_due_date/gi))
                                     sublistFields[id].updateDisplaySize({
                                         height: 60,
                                         width: 8
                                     });
...
//                        if(line <= 1000){ // commented by: jeromemorden | 20220404
             ...
             form.clientScriptModulePath = './se_cs_generalWOschedule_v4.0';
