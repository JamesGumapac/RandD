            ...
            jQuery(document).ready(function() {
                ...
                for(var line=0; line < lineCount; line++){
                    ...
                    var woPriority = parseFloatOrZero(currentRecord.getSublistValue({
                        sublistId: 'custpage_sublist',
                        fieldId: 'custcol_cr_cntm_priority1',
                        line: line
                    }));
                    ...
                    console.log('>>>>', JSON.stringify({ line: line, soType: soType }))
                    var style = '';
                    ...
                    if (dueDate && latesProjEndDate) {
                        if (new Date(dueDate) < new Date(latesProjEndDate)) {
                            ...
                            jQuery('#custpage_sublistrow' + line).find('td.uir-list-row-cell:nth-child(7)').attr('style', style); // 6-Latest Projected End Date (RED TEXT BOLD)
                        }
                    }
...
//			console.log(sublistId, fieldId, lineNum);
             var woId = currentRecord.getSublistValue({
                 sublistId: sublistId,
                 line: lineNum,
                 fieldId: 'custcol_internalid'
             });
			var sequence = currentRecord.getSublistValue({
				sublistId: sublistId,
				line: lineNum,
				fieldId: 'custcol_cr_cntm_seq_no'
			});
...
//             if(duplicatedIds.indexOf(editedId) >= 0)
  //               alert('This Work Order has duplicate on this list.  Only the most top line will be processed.');
			 ...
			 if(updatedIds.length >= 10 && updatedIds.indexOf(editedId) < 0)
			 	alert("You can update 10 lines only at a time. Changes made for this line will not save.");
             else if(updatedIds.indexOf(editedId) < 0)
