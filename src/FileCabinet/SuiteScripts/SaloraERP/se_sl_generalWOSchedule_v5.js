    ...
    */
    ...
    const firstPage = context => {
        ...
        try {
            ...
            var searchId = script.getParameter('custscript_gwos_search_param_v5');
            var editedFields = script.getParameter('custscript_gwos_editfields_param_v5');
            ...
            */
            ...
            var searchResults = ((ssId, par) => {
                ...
                */
                ...
                // Get result limit
                var script = runtime.getCurrentScript();
                var limit = par.custpage_page == 'csv' ? '' : parseInt(script.getParameter('custscript_gwos_linelimit_param_v5'));
...
//                getIdFilter = sequence.map(a=>`[["custrecord_cntm_work_order", "anyof", "${a.wo}"], "AND", ["custrecord_cntm_seq_no", "equalto", "${a.sequence}"]]`)
                        ...
                        searchResults[0].columns.forEach((col, i) => {
                            ...
                            var displayType = label.match(/display\[/gi)? label.match(/display\[.*?\]/gi)[0].replace(/display\[|\]/gi, ''): '';
                             ...
                             if(displayType == 'CSV'){
                                 displayType = 'HIDDEN';
                                 csvOnly = true;
                                 label = label.replace('display[CSV]', '')
                             }
                            ...
                            if (label != woPriority1_2FormulaLabel) { // Dont add the WOPrio1 + WOPrio2 to the UI cols
								...
								if(!csvOnly){
									...
									// Added by jeromemorden | 01/20/22
									if (id.match(/priority|weekend_hours/gi))  //added priorty|
									...
									if (id.match(/quantity|qfactor|qtyoperation|no_of_panel|total_num_cores|boards_per_panel|qty_due/gi)) {
										sublistFields[id].updateDisplaySize({
											height: 60,
											width: 6
										});
                                ...
                                // end
                                if (!id.match(/internalid/gi) && (displayType != 'HIDDEN' || csvOnly))
									csvText += `"${label}",`;
                            }
                        });
            ...
            form.clientScriptModulePath = './se_cs_generalWOSchedule_v5';
