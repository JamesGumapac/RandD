...
{name: 'Current DB-MCaprio-PC', search: 'customsearch_cdb_mcaprio_pc_cas'}, //customsearch_cdb_mcaprio_pc_v2
{name: 'Current DB-DB-FINAL', search: 'customsearch_cdb_final2_cas'}, // customsearch_cdb_final2_v2
{name: 'Current DB-DB-DRILL1', search: 'customsearch_cdb_drill12_cas'}, // customsearch_cdb_drill12_v2
{name: 'Current DB-DB-VIAFILL', search: 'customsearch_cdb_viafill2_cas'}, // customsearch_cdb_viafill2_v2
{name: 'Current DB-DB-ETCH', search: 'customsearch_cdb_viafill2_v2_cas'},// customsearch_cdb_etch2_v2
{name: 'Current DB-DB-PLATING', search: 'customsearch_cdb_plating2_cas'},// customsearch_cdb_plating2_v2
{name: 'Current DB-DB-Layup', search: 'customsearch_cdb_layup2_cas'}, // customsearch_cdb_layup2_v2
{name: 'Current DB-DB-ET', search: 'customsearch_cdb_et2_cas'}, // customsearch_cdb_et2_v2
{name: 'Current DB-DB-AOI', search: 'customsearch_cdb_aoi2_cas'}, // customsearch_cdb_aoi2_v2
{name: 'Current DB-DB-OuterLayers', search: 'customsearch_cdb_outerlayers2_cas'}, // customsearch__cdb_outerlayers2_v2
{name: 'Current DB-DB-SANDING', search: 'customsearch_cdb_sanding2_cas'}, // customsearch_cdb_sanding2_v2
{name: 'Current DB-DB-HDI', search: 'customsearch_cdb_hdi2_cas'}, // customsearch_cdb_hdi2_v2
{name: 'Current DB-DB-InnerLayers', search: 'customsearch_cdb_innlays2_cas'}, // customsearch_cdb_innlays2_v2
{name: 'Current DB-DB-SolderMask', search: 'customsearch_cdb_soldmask2_cas'}, // customsearch_cdb_soldmask2_v2
{name: 'Current DB-DB-Ni-Au', search: 'customsearch_cdb_ni_au2_cas'}, // customsearch_cdb_ni_au2_v2
{name: 'Current DB-DB-LASERDRILL', search: 'customsearch_cdb_laserdrill2_cas'}, // customsearch_cdb_laserdrill2_v2
{name: 'Current DB-DB-DES', search: 'customsearch_cdb_des2_cas'}, // customsearch_cdb_des2_v2
{name: 'Current DB-DB-Inline', search: 'customsearch_cdb_inline2_cas'}, // customsearch_cdb_inline2_v2
{name: 'Current DB-DB-DRILL2', search: 'customsearch_cdb_drill22_cas'}, // customsearch_cdb_drill22_v2
{name: 'Current DB-DB-ROUT', search: 'customsearch_cdb_rout2_cas'}, // customsearchcdb_rout2_v2
{name: 'Current DB-DB-ELEC-CU', search: 'customsearch_cdb_elec_cu2_cas'}, // customsearch_cdb_elec_cu2_v2
{name: 'Current DB-DB-DEV-CCLEAN', search: 'customsearch_cdb_dev_cclean2_cas'}, // customsearch_cdb_dev_cclean2_v2
{name: 'Current DB-DB-DESMEAR-DB', search: 'customsearch_cdb_desmear_db2_cas'}, // customsearch_cdb_desmear_db2_v2
{name: 'Current DB-Enepigdashboard', search: 'customsearch_cdb_enepig_cas'} // customsearch_cdb_enepig_v2
];
   ...
   if (context.request.method == 'GET') {
         ...
         // Add sublist that will show results
            var form = serverWidget.createForm({
             title : "PCB    "+ DASHBOARD_OBJECT_ARRAY[DASHBOARD_SELECTED].name,
             hideNavBar : true
         });
         form.clientScriptModulePath = 'SuiteScripts/manufacturingDashboard_CS2_v2_cas.js';
                 
         var sublist = buildFormSublist(form, DASHBOARD_OBJECT_ARRAY);

         // Get subset of data to be shown on page
         var addResults = fetchSearchResult2(DASHBOARD_OBJECT_ARRAY[DASHBOARD_SELECTED].name, DASHBOARD_OBJECT_ARRAY[DASHBOARD_SELECTED].search);
...
**/
 ...
 function fetchSearchResult2(searchName, searchId) {
    try{
        var searchObj = search.load({id : searchId});
        /**if (searchName == "Current DB-DB-FINAL") {
            var formulaString = "formulanumeric: CASE WHEN (INSTR({custrecord_cntm_cso_operaton}, 'EX-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'FP-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, '8B-') != 0) THEN 1 ELSE 0 END";
            var filterArray = searchObj.filterExpression;
            filterArray.push('AND');
            filterArray.push([formulaString, "equalto", "1"]);
            searchObj.filterExpression = filterArray;  
        }
        else if (searchName == "Current DB-DB-DRILL1") {
            var formulaString = "formulanumeric: CASE WHEN (INSTR({custrecord_cntm_cso_operaton}, 'D3-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'EZ-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'FW-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'D4-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, '5B-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'G6-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'G5-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'G4-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, '78-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, '7B-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'D5-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'D6-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, '7A-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'X1-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, 'XY-') != 0 OR INSTR({custrecord_cntm_cso_operaton}, '7H-') != 0) THEN 1 ELSE 0 END";
            var filterArray = searchObj.filterExpression;
            filterArray.push('AND');
            filterArray.push([formulaString, "equalto", "1"]);
            searchObj.filterExpression = filterArray;  
        ...
        var searchObjResultCount = searchObj.runPaged().count;
        log.debug("searchObj result count in runSearch function is:", searchObjResultCount);
        var resultRaw = searchObj.run().getRange({start: 0, end: 50});
        //log.debug("searchObj result:", JSON.stringify(result));
        var resData =
            resultRaw.map(function(result) {
                return {
                    seqNo: result.getValue({name: "custrecord_cntm_seq_no", summary: "GROUP"}),
                    internalId: result.getValue({name: "internalid", summary: "MIN"}),
                    woDueDate: result.getValue({name: "custrecord_cntm_cah_woduedate", join: "CUSTRECORD_CNTM_CSO_PARENTREC", summary: "GROUP"}),
                    woName: result.getValue({name: "custrecord_cntm_work_order", summary: "MIN"}),
                    woId: result.getValue({name: "internalid", join: "CUSTRECORD_CNTM_WORK_ORDER", summary: "GROUP"}),
                    //quantity: result.getValue({name: "custrecord_cntm_cah_total_lot_quantity", join: "CUSTRECORD_CNTM_CSO_PARENTREC", summary: "GROUP"}),
                    panel: result.getValue({name: "custrecord_cntm_num_panels", join: "CUSTRECORD_CNTM_CSO_PARENTREC", summary: "GROUP"}),
                    customer: result.getText({name: "custrecord_cntm_cah_customer", join: "CUSTRECORD_CNTM_CSO_PARENTREC", summary: "GROUP"}),
                    operation: result.getValue({name: "custrecord_cntm_cso_operaton", summary: "GROUP"}),
                    percentCompleted: result.getValue({name: "custbody_wo_percent_completed", join: "CUSTRECORD_CNTM_WORK_ORDER", summary: "GROUP"}),
                    commentsDash: result.getValue({name: "custbody_comments_for_dash", join: "CUSTRECORD_CNTM_WORK_ORDER", summary: "GROUP"}),
                    woPriority: result.getValue({name: "custbody_rda_wo_priorty", join: "CUSTRECORD_CNTM_WORK_ORDER", summary: "GROUP"}),
                    woTranDate: result.getValue({name: "trandate", join: "CUSTRECORD_CNTM_WORK_ORDER", summary: "MIN"}),
                    quantity: result.getValue({name: "formulanumeric", summary: "SUM", formula: "TO_NUMBER({custrecord_cntm_cso_quantity_good})"})
                };
            }) || [];
        log.debug("resData:", JSON.stringify(resData));
        ...
        resData.forEach(function(v) {
            if (i.indexOf(v.woId) < 0) {
                ...
                groupedByWoId.push({
                    'woId' : v.woId,
                    'data' : resData.filter(function(itm) {return itm.woId == v.woId;})
                });
            }
        });
        ...
        var resultArray = [];
        var q = 0;
        for (var z = 0; z < groupedByWoId.length; z++) {

            // woPriority
            var woPriority = groupedByWoId[z].data[0].woPriority;
            if (!woPriority) {
            ...
            // jobNumber
            var jobAndNoBoardsArray = givenWOidReturnJobFieldTextAndNoBoardsPerPanel(groupedByWoId[z].woId);
            var jobField = jobAndNoBoardsArray[0] ? jobAndNoBoardsArray[0] : 'N/A';

            // woName substring
            var workOrder = groupedByWoId[z].data[0].woName;
            var workOrderSubString = workOrder.substring(workOrder.indexOf('#'), workOrder.length);

            //entity string
            var entityStringResult = groupedByWoId[z].data[0].customer;
            if(!entityStringResult){
                var entityString = 'N/A';
            }
            else{
 ...
 function fetchSearchResult(searchId) {
     ...
     for(var z = 0; z < result.length; z++){
         ...
         var woPriority = result[z].getValue({name: "custbody_rda_wo_priorty", join: "CUSTRECORD_OPERATION_LINE_WO"});
         if(!woPriority){
             woPriority = Math.floor(5).toFixed(0);
         }
         else{
        ...
        log.debug("predecessorscompqty",predecessorsCompQty);
         ...
         var percentCompleted = result[z].getValue({name: "custbody_wo_percent_completed", join: "CUSTRECORD_OPERATION_LINE_WO"});
         var woPriority1 = result[z].getValue({name: "formulanumeric"});
         var woPriority2 = result[z].getValue({name: "custbody_rda_wo_priorty_2", join: "CUSTRECORD_OPERATION_LINE_WO"})
...
**/                 
 ...
 function buildFormSublist(form, dashboardarray){
                 ...
                 sublist.addField({id : 'percentcomplete', label : 'Percent Completed', type : serverWidget.FieldType.TEXT});
 ...
 function setSublistFunction(sublist, result){
     ...
     // Set data returned to columns
                 ...
                 result.forEach(function (result) {
                     ...
                     sublist.setSublistValue({id: 'duedate', line : j, value : result.duedate ? result.duedate : format.format({value: '01/01/1980', type: format.Type.DATE})});
