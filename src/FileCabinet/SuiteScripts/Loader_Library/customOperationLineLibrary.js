 ...
 */
define(['N/search', './lib/moment.min', './lib/ns.utils','N/record'], (search, moment, ns_utils,record) => {
...
// totalNumberOfCores = WORK ORDER main line field (ID: custbody_total_num_cores)
    const deleteAndCreateCustomOperationLines = (woId, currUser, quantity, goodnumofpanels, totalnumofcores) => {
        try{
            // DELETE LINES START
        ...
        if(target[woId].gate1.custOperationId){
            targetGateObjReturn.custbody_target_gate_second = target[woId].gate1.custOperationId;
        }

        if(target[woId].gate2.custOperationId){
            targetGateObjReturn.custbody_target_gate_first = target[woId].gate2.custOperationId;
        }
    ...
    const updateExistingCustomOperationLinesUponCreationOfWorkCompletion = (woId, currentUser, startOperation, endOperation, noOfPanels, goodNumberOfPanels, totalNumberOfCores) => {
        try{
            var date = moment(ns_utils.dateNowByCompanyTimezone()).format('M/D/YYYY hh:mm a');
            ...
            for (i in custOperationsSucceeding) {
                ...
                if (i == 0) {
                    ...
                    custOperationsSucceeding[i].projected = moment(new Date(date)).add(custOperationsSucceeding[i].minstoadd, 'minutes').format('M/D/YYYY hh:mm a')
                } 
                else {
                    //var minsToAdd = custOperationsSucceeding[i-1].runtime + custOperationsSucceeding[i-1].setuptime
                    custOperationsSucceeding[i].startdate       = ''
                    custOperationsSucceeding[i].enddate         = ''
                    custOperationsSucceeding[i].prev_projected  = custOperationsSucceeding[i-1].projected
                    custOperationsSucceeding[i].projected       = moment(new Date(custOperationsSucceeding[i].prev_projected)).add(custOperationsSucceeding[i].minstoadd, 'minutes').format('M/D/YYYY hh:mm a')
                }
            }
