        ...
        try {
           ...
           for(var i=0;i<a;i++)
           {
            ...
            for(var i=0;i<serialIdArray.length;i++){
              ...
              var all_serial = myCache.get({
                key:'arrayForSerial',
                ttl: 300
            });
            log.debug("all_serial",all_serial)

            var serarr=[];
          
            var len = all_serial.split(",").length;
            for(var i=0;i<len;i++)
            {
               if(all_serial.split(",")[i] !="initial" && all_serial.split(",")[i]!="aarrayForSerial" )
              {  serarr.push(all_serial.split(",")[i]) }
            }

            log.debug("final array",serarr)
            ...
            //         return {
            //             "success" : "false",
            //             "datain" : "Serial id " +obj+ " already exists"
            //         };
            //    // }
