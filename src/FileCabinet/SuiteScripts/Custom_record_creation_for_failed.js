 ...
 * 
 */
define(["N/search", "N/record", "N/runtime", "N/url", "N/https"/*,"./customOperationLineLibrary.js"*/], function (// Loader_Library
    ...
    function getInputData() {
        ...
        var workOrderid = runtime.getCurrentScript().getParameter({
            name: "custscript_cntm_woid_cust_rec_c",
        });
        
        var missingLot = runtime.getCurrentScript().getParameter({
           name: "custscript_cntm_missing_lot",
       });
       log.debug("workOrderid "+workOrderid,"missingLot "+missingLot);
       lotArray=getMissingLot(workOrderid,missingLot);
        //lotArray = getLotArray(workOrderid);
        log.debug('lotArray :',lotArray)
   
        return lotArray;
    }
    function getMissingLot(wo,missingLot) {
       var templotRecIdArray = [];
       var customrecord_cntm_lot_creationSearchObj = search.create({
           type: "customrecord_cntm_lot_creation",
           filters: [["custrecord_cntm_lot_wonum", "anyof", wo],"AND",["custrecord_cntm_lot_lotnumber","equalto",missingLot]],
           columns: [
               search.createColumn({
    ...
    function map(mapContext) {
        ...
        fieldId: "custrecord_cntm_lot_lotnumber",
                ...
                var cumScrapQty = lotRecLookUp.custrecord_cntm_cumulative_scrap_qty;/*rec.getValue({
        fieldId: "custrecord_cntm_cumulative_scrap_qty",
      });*/
                var prevScrap = lotRecLookUp.custrecord_cntm_previous_scrap;/*rec.getValue({
        fieldId: "custrecord_cntm_previous_scrap",
      });*/
                log.debug("lot", lot);
                var noOfPanels = lotRecLookUp.custrecord_cntm_num_of_panels;/*rec.getValue({
        fieldId: "custrecord_cntm_num_of_panels",
                ...
                var lotQty = lotRecLookUp.custrecord_cntm_stock_lot_qty;/*rec.getValue({
